...
select ENumber, sum(PCount) from tProduction where PDate like '2020-01%' and PCount >= 500 group by ENumber order by sum(PCount) desc;
...
select DNumber, count(*) from tEmployee where DNumber not like 'NULL' group by DNumber having count(*) >= 5;
