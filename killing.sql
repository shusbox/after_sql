...
select count(*) from Customer group by custid;
...
select max(targetid) from Order_Kill group by targetid having count(*) >= 2;

# 고객별로 요청한 타겟의 총 명수와 총 청부비용을 구하시오.
select custid, count(*), sum(t.price)
from Order_Kill ok join Target t on ok.targetid = t.targetid
group by ok.custid;
